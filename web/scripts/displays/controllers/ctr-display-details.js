    ...
    function ($scope, $q, $state, displayFactory, display, $loading, $log,
      ...
      $scope.requestScreenshot = function() {
        ...
        return display.requestScreenshot(displayId)
          ...
          .catch(function(err) {
            ...
            $scope.screenshot = { error: 'requesting' };
      ...
      $scope.loadScreenshot = function() {
        ...
        return display.loadScreenshot(displayId)
          ...
          .catch(function(err) {
            display.screenshotLoading = false;
            $scope.screenshot = { error: 'loading' };
      ...
      $scope.screenshotState = function(display) {
        ...
        if(!display || $scope.displayService.statusLoading || $scope.displayService.screenshotLoading) {
          return 'loading';
        }
        ...
        else if(display.playerName !== "RisePlayerElectron" || display.playerVersion <= '2017.01.10.17.33') {
          return 'upgrade-player';
        }
        else if(!$scope.displayService.hasSchedule(display)) {
          return 'no-schedule';
        }
        else if(statusFilter(display) === 'offline' && $scope.screenshot && $scope.screenshot.lastModified) {
      ...
      $scope.reloadScreenshotDisabled = function(display) {
        return $scope.displayService.statusLoading ||
          ...
          ['no-screenshot-available', 'screenshot-loaded'].indexOf($scope.screenshotState(display)) === -1;
      };
